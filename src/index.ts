...
import { getQuote, buildSwapTransaction, QuoteResponse } from './jupiter.js';
...
function quotePrice(quote: QuoteResponse): number {
...
async function executeSwap(
    ...
    if (dryRun) {
        ...
        console.log(
            `[DRY-RUN] Would swap ${quote.inAmount} ${quote.inputMint} → ${quote.outAmount} ${quote.outputMint} (price ${price.toFixed(6)})`,
        );
        return undefined;
    }
...
interface Position {
    entrySolLamports: bigint;  // SOL spent (in lamports)
    entryUsdtzUnits: bigint;   // USDTZ received (raw units)
    entryPrice: number;        // USDTZ per lamport at time of buy
}
    ...
    while (true) {
        try {
            if (position === null) {
                ...
                console.log(
                    `[IDLE] Quote: ${config.ENTRY_SOL} SOL → ${buyQuote.outAmount} USDTZ units ` +
                ...
                console.log(
                    `[POSITION OPEN] Entry price ${entryPrice.toFixed(6)} | TP @ ${(entryPrice * takeProfitMul).toFixed(6)} | SL @ ${(entryPrice * stopLossMul).toFixed(6)}`,
                );
                ...
                console.log(
                    `[MONITOR] PnL ${pnlPct >= 0 ? '+' : ''}${pnlPct.toFixed(2)}% | ` +
                ...
                if (shouldTakeProfit || shouldStopLoss) {
                    ...
                    console.log(
                        `[POSITION CLOSED] Reason: ${reason} | Final PnL: ${pnlPct >= 0 ? '+' : ''}${pnlPct.toFixed(2)}%`,
                    );
                    position = null;
                }
            }
